...
@RequestMapping("/bomAirport") // The paths for HTTP requests are relative to this base path.
@CordaSerializable
class BomController(@Qualifier("bomAirportConnection")  rpc: NodeRPCConnection) {
    ...
    private fun getAllTxOne(@RequestParam(value = "airwayBillNumber") airwayBillNumber: String): List<AmsController.Information> {
        ...
        var allInformation = mutableListOf<AmsController.Information>()
        return if (response.isNotEmpty()){
            for (state in response) {
                val information = AmsController.Information(state.awbId, state.uniqueMessageTypeNumber, state.orgLoc, state.dstLoc, state.pkgs, state.weight, state.status, state.timeStamp)
    ...
    private fun getTxDetailsOne(@RequestParam(value = "airwayBillNumber") airwayBillNumber: String): AmsController.Information {
        val generalCriteria = QueryCriteria.VaultQueryCriteria(Vault.StateStatus.UNCONSUMED)
        ...
        if (response.isNotEmpty()) {
            return AmsController.Information(response.first().awbId, response.first().uniqueMessageTypeNumber, response.first().orgLoc, response.first().dstLoc, response.first().pkgs, response.first().weight, response.first().status, response.first().timeStamp)
        } else {
            throw IllegalArgumentException("There is NO entry with this airway number")
        }
    }
